...
super_variable link_super_variable(const char *name, size_t size)
{
    ...
    super_variable p = (super_variable)mmap(NULL, \
            get_full_size(size), \
            PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if(p == MAP_FAILED) goto END;
...
int read_super_variable(super_variable p, void *buf, size_t size)
{
    ...
    while(qh != qt && \
            memcmp(&p->b_timestamp[qh], &p->timestamp, sizeof(struct timespec)))
        qh = ((qh + 1) & (CIRCLE_QUEUE_LENGTH - 1)); 
    p->qhead = qh; p->qtail = qt; 
    release_meta_lock(p);
    
    /* mem copying may be costly */
    if(qh == qt) return -1; // Nothing to read
    memcpy( buf, \
            p->data + qh * size, \
            size);
...
int write_super_variable(super_variable p, void *data, size_t size)
{
    ...
    err_code = acquire_meta_lock(p);
    if(err_code) // We haven't acquire a lock here, so just return is ok
        return err_code;
    uint8_t qh = p->qhead, qt = p->qtail;
    ...
    p->b_timestamp[tmp] = p->timestamp = ts;

    /* Don't forget to write into the metablock and unlock */
END_METALOCK: 
