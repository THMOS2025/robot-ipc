...
当设置 `data_format` 后，会使用 `ctypes.Structure` 的固定内存布局进行传输，可与 C/C++ 结构体兼容。
...
- name：用于标识变量的字符串
- max_arg_size：传递参数的最大大小（pickle 后）
- max_ret_size：返回响应的最大大小（pickle 后）
- `__call__(*args, **kwargs)`：像普通函数一样调用实例
- `.get_response()`：等待响应并返回。

当设置 `arg_format` / `ret_format` 后，参数与返回值将使用固定结构体布局，而不是 pickle。
...
- 启动后可以添加其他函数（`.attach()`），但尚未经过全面测试，可能会失败。
